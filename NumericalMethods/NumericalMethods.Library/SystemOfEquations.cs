...
namespace NumericalMethods.Library
{
    public class SystemOfEquations : ISystemOfEquations
    {
        ...
        private double[] SolveLinearEquations(string[] coefficients)
        {
            ...
            {
                rowsMatrix[i] = (double[])Array.ConvertAll(coefficients[i].Split(new char[] { ' ', ',' }), double.Parse);

            }
            return GetReduceForm(rowsMatrix);
        }

        private double[] GetReduceForm(double[][] rowsMatrix)
        {
